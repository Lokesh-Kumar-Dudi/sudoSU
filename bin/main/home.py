...
class segy():
    ...
    def get_trace_hdrs(self,filename,trace_no):
        ...
        trace_info = open(home1+"/sudoSU/bin/main/trace_hdrs_info.txt",'r')
        trace_hdrs={}
        size_trace=os.stat(filename)[6]-3200-400
        n_traces=int(size_trace/(n_samples*4+240))
        if trace_no>n_traces:
            trace_hdrs['Invalid Trace number ']=str(trace_no)
        else:
...
class home:
    def __init__(self,parent):
        self.parent = parent
        self.parent.segy = segy()
        self.filename = self.parent.props.segy_file
        self.parent.home  = self.parent.builder.get_object("home") 
        self.parent.tape_name  = self.parent.builder.get_object("tape_name")
        self.parent.tape_size = self.parent.builder.get_object("tape_size")
        self.parent.binary_hdrs = self.parent.builder.get_object("binary_headers")
        self.parent.text_hdrs=self.parent.builder.get_object("text_hdrs")
        self.parent.trace_hdrs = self.parent.builder.get_object("trace_hdrs")
        self.parent.trace_no = self.parent.builder.get_object("trace_no")
    
    def get_string(self,str1,str2,n):
        while((len(str1)+len(str2)< n)):
            str1+=" "

        return str1+str2

    def set_assets(self,filename):
        if self.parent.props.curr_project==None:
            self.parent.tape_name.set_text("")
            self.parent.binary_hdrs.set_text("")
            self.parent.text_hdrs.set_text("")
            self.parent.tape_size.set_text("")
        elif filename==None:
            self.parent.tape_name.set_text("No SEGY File found in Project")
        else:
            self.filename = filename
            self.parent.tape_name.set_text(ntpath.basename(filename))
            self.parent.tape_size.set_text(self.parent.segy.get_size(filename))
            binary_hdrs = self.parent.segy.get_binary_hdrs(filename)
            for i in binary_hdrs:
                self.parent.binary_hdrs.set_text(self.parent.binary_hdrs.get_text()+i+"-----"+binary_hdrs[i]+"\n")
            text_hdrs  = self.parent.segy.get_ebcdic_hdrs(filename)
            for i in text_hdrs:
                self.parent.text_hdrs.set_text(self.parent.text_hdrs.get_text()+i+"\n"  )  
    def display_trace_hdrs(self,filename):
        self.parent.trace_hdrs.set_text("")
        if self.parent.props.curr_project==None:
            self.parent.trace_hdrs.set_text("No Project opened")
        elif filename==None:
            self.parent.trace_hdrs.set_text("No SEGY file found in the Project")
        else:    
            trace_hdrs = self.parent.segy.get_trace_hdrs(self.filename,int(self.parent.trace_no.get_text()))   
            for i in trace_hdrs:
                    self.parent.trace_hdrs.set_text(self.parent.trace_hdrs.get_text()+i+"-----"+trace_hdrs[i]+"\n")
