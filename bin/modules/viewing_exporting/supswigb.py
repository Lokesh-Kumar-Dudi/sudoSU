...
class supswigb(Gtk.Box):
    def __init__(self,parent):
        self.parent = parent
        self.glade_file=str(Path(__file__).parent/ "supswigb.glade")
        self.builder = Gtk.Builder()
        self.builder.add_from_file(self.glade_file)
        self.main_box = self.builder.get_object("supswigb")
        self.title_box = self.builder.get_object("tab_title")
        self.builder.connect_signals(self)
        self.list = ['label1','label2','title','titlecolor','perc']
        ...
        for item in self.list:
            ...
            if type(obj)==type(Gtk.Entry()):
                if obj.get_text()!="":
                    ...
                    pass
        if self.parameters!="":
            self.command = "supswigb < infile "+ self.parameters + " >" + "outfile"
    ...
    def help(self, widget):
        ...
        help_dlg.set_title("SUPSWIGB Help")
        help_dlg.show_all()
        

    def destroy(self, widget):
        self.main_box.destroy()
        self.parent.tabs['SUPSWIGB'] = 0


class nb_page(Gtk.Box):
    def __init__(self,parent):
        self.main_class = supswigb(parent)
