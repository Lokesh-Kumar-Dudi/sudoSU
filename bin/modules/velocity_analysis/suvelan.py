...
class suvelan(Gtk.Box):
    def __init__(self,parent):
        self.parent = parent
        self.glade_file=str(Path(__file__).parent/ "suvelan.glade")
        self.builder = Gtk.Builder()
        self.builder.add_from_file(self.glade_file)
        self.main_box = self.builder.get_object("suvelan")
        self.title_box = self.builder.get_object("tab_title")
        self.builder.connect_signals(self)
        self.list = ['nv','dv','fv','anis1','anis2','smute','dtratio','nsmooth','pwr']
    ...
    def set_parameters(self, button):
        ...
        if self.parameters!="":
            self.command = "suvelan < infile "+ self.parameters + " >" + "outfile"
            self.append_wf_cmd ="suvelan < $infile "+ self.parameters + " >" + "$prefix.$infile"
    ...
    def run(self, widget):
        ...
        if len(self.parent.input_files)==len(self.parent.output_files):
            ...
            for i,input_file in enumerate(self.parent.input_files):
                cmd = "suvelan"+ " < " + input_file + " "+self.parameters \
    ...
    def help(self, widget):
        ...
        help_dlg.set_title("SUVELAN Help")
        help_dlg.show_all()
        

    def destroy(self, widget):
        self.main_box.destroy()
        self.parent.tabs['SUVELAN'] = 0


class nb_page(Gtk.Box):
    def __init__(self,parent):
        self.main_class = suvelan(parent)
