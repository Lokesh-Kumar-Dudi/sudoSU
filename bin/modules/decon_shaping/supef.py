...
class supef(Gtk.Box):
    def __init__(self,parent):
        self.parent = parent
        self.glade_file=str(Path(__file__).parent / "supef.glade")
        self.builder = Gtk.Builder()
        self.builder.add_from_file(self.glade_file)
        self.main_box = self.builder.get_object("supef")
        self.title_box = self.builder.get_object("tab_title")
        self.builder.connect_signals(self)
        self.list = ['dt','cdp','minlag','maxlag','pnoise','mincorr','maxcorr','wienerout','mix','outpar']
        ...
        for item in self.list:
            ...
            if type(obj)==type(Gtk.Entry()):
                ...
                if obj.get_active():
                    ...
                    pass
        obj = self.builder.get_object('method')
        tree_iter = obj.get_active_iter()
        if tree_iter is not None:
            model = obj.get_model()
            row_id = model[tree_iter][0]
            self.parameters +="method=" + row_id + " "
        if self.parameters!="":
            self.command = "supef < infile "+ self.parameters + " >" + "outfile"
            self.append_wf_cmd ="supef < $infile "+ self.parameters + " >" + "$prefix.$infile"
    ...
    def run(self, widget):
        ...
        if len(self.parent.input_files)==len(self.parent.output_files):
            ...
            for i,input_file in enumerate(self.parent.input_files):
                cmd = "supef"+ " < " + input_file + " "+self.parameters \
    ...
    def destroy(self, widget):
        ...
        self.parent.tabs['SUPEF'] = 0

class nb_page(Gtk.Box):
    def __init__(self,parent):
        self.main_class = supef(parent)
